...
export default async function BlogListing() {
  await connectDB();
  const posts = (await BlogPost.find({ published: true }).sort({ createdAt: -1 })) as BlogPreview[];
        ...
        <header className="bg-brand-emerald py-16 sm:py-20 mb-10 md:mb-12 relative overflow-hidden">
          <div className="container mx-auto px-4 text-center relative z-10">
            <h1 className="text-4xl md:text-6xl font-bold text-white mb-6 font-heading">
              {cms['blog.hero.title'] || 'Investment Insights'}
            </h1>
          ...
          {posts.length === 0 ? (
            <div className="text-center py-32 bg-white rounded-3xl border border-dashed border-zinc-200">
              <h3 className="text-xl font-bold text-zinc-700 mb-2">
                {cms['blog.empty.title'] || 'Insights Coming Soon'}
              </h3>
              <p className="text-zinc-400 font-medium">
                {cms['blog.empty.text'] || 'Our editorial team is preparing expert content.'}
              </p>
            </div>
          ) : (
              ...
              {posts.map((post) => (
                ...
                >
                  ...
                  <div className="p-8">
                    ...
                    <Link
                      href={`/blog/${post.slug}`}
                      className="inline-flex items-center gap-2 text-brand-emerald font-bold text-xs uppercase tracking-widest hover:gap-4 transition-all duration-300"
                    >
